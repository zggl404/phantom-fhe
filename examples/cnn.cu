...
int main(int argc, char **argv)
{
	...
	if (dataset == 10)
		...
		throw std::invalid_argument("dataset number is not correct");
...
void ResNet_cifar10_seal_sparse(size_t layer_num, size_t start_image_id, size_t end_image_id)
{
	...
	int remaining_level = 2 + 3; // Calculation + s2c
	...
	parms.set_special_modulus_size(n_special_primes);
	...
	bootstrapper_1.set_slim_relu(enable_relu);
	bootstrapper_2.set_slim_relu(enable_relu);
	bootstrapper_3.set_slim_relu(enable_relu);
	...
	for (size_t image_id = start_image_id; image_id <= end_image_id; image_id++)
	{
		...
		ctxt = cnn.cipher();
		if (ctxt.coeff_modulus_size() - 1 < 16 + 3 + 3)
		{
			throw std::invalid_argument("No levels.");
		}
		while (ctxt.coeff_modulus_size() - 1 > 16 + 3 + 3)
		{
		...
		std::cout<<"the cur_level is "<<cur_level<<std::endl;
		std::cout<<"the modulus is "<<std::endl;
		

		for(int i = 0;i<modulus.size();i++)
		{
			std::cout<<modulus[i].value()<<"        "<<log2(modulus[i].value())<<std::endl;
		}
		auto first_modulus = modulus[cur_level - 1];
		PhantomPlaintext scaler;
		double scale_change = pow(2.0, 46) * ((double)first_modulus.value()) / ctxt.scale();
		std::cout<<"the scale change is "<<scale_change<<std::endl;
		std::cout<<"the scale change of log is "<<log2(scale_change)<<std::endl;
...
void ResNet_cifar100_seal_sparse(size_t layer_num, size_t start_image_id, size_t end_image_id)
{
	...
	for (size_t image_id = start_image_id; image_id <= end_image_id; image_id++)
	{
		...
		// scaling factor ~2^51 -> 2^46
		ctxt = cnn.cipher();
		if (!std::isfinite(ctxt.scale()) || ctxt.scale() <= 0.0)
			throw std::runtime_error("invalid ctxt.scale before layer-0 rescale");
		const auto &coeff_modulus = context.get_context_data(ctxt.chain_index()).parms().coeff_modulus();
		if (coeff_modulus.empty())
			throw std::runtime_error("empty coeff_modulus during layer-0 rescale");
		const double q_last = static_cast<double>(coeff_modulus.back().value());

		PhantomPlaintext scaler;
		double scale_change = std::ldexp(1.0, 46) * q_last / ctxt.scale();
		if (!std::isfinite(scale_change) || scale_change <= 0.0)
			throw std::runtime_error("invalid scale_change during layer-0 rescale");
		ckks_evaluator.encoder.encode(1, scale_change, scaler);
		ckks_evaluator.evaluator.mod_switch_to_inplace(scaler, ctxt.chain_index());
		ckks_evaluator.evaluator.multiply_plain_inplace(ctxt, scaler);
		ckks_evaluator.evaluator.rescale_to_next_inplace(ctxt);
