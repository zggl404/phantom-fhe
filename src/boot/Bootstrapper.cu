...
void Bootstrapper::genfftcoeff_3()
{ // not yet
  fftcoeff1.resize(slot_vec.size());
  fftcoeff2.resize(slot_vec.size());
...
void Bootstrapper::geninvfftcoeff_3()
{ // not yet
  invfftcoeff1.resize(slot_vec.size());
  invfftcoeff2.resize(slot_vec.size());
...
void Bootstrapper::sfl(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  ...
  auto curr_level = ckks->context->get_context_data(tmpct.chain_index()).chain_depth();
...
void Bootstrapper::sfl_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{ // not yet
  ...
  const auto modulus = ckks->context->first_context_data().parms().coeff_modulus();
  auto curr_level = ckks->context->get_context_data(tmpct2.chain_index()).chain_depth();
...
void Bootstrapper::sfl_half_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{ // not yet
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::sflinv_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{ // not yet
  int div_part1 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part1) / 2.0);
  int div_part3 = logn - div_part1 - div_part2;
...
void Bootstrapper::bootstrap_sparse(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  ...
  {
    const auto modulus = ckks->context->first_context_data().parms().coeff_modulus();
    auto curr_level = ckks->context->get_context_data(modrtn.chain_index()).chain_depth();
...
void Bootstrapper::bootstrap_sparse_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  ...
  // PhantomPlaintext tmpplain_1;

  // ckks->encoder.encode((1 << (logNh - logn)), cipher.scale(), tmpplain_1);  // scale of tmpplain = Delta
  // ckks->evaluator.mod_switch_to_inplace(tmpplain_1, cipher.params_id());
  // ckks->evaluator.multiply_plain_inplace(cipher, tmpplain_1);  // scale of cipher = Delta^2
  // ckks->evaluator.rescale_to_next_inplace(cipher);  // scale of cipher = Delta^2 / q0

  // for (auto i = logn; i < logNh; i++) {
  //   ckks->evaluator.add_inplace(cipher, cipher);
  // }
  ...
  for (auto i = logn; i < logNh; i++)
  {
    ckks->evaluator.add_inplace(rtncipher, rtncipher);
  }
}
...
void Bootstrapper::slim_modraise_inplace(PhantomCiphertext &cipher)
{
  if (cipher.size() != 2)
  {
    throw invalid_argument("Ciphertexts of size 2 are supported only!");
...
void Bootstrapper::slim_sfl_full_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::slim_sflinv_full_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  int div_part1 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part1) / 2.0);
  int div_part3 = logn - div_part1 - div_part2;
...
void Bootstrapper::slim_sfl_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::slim_sflinv_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  int div_part1 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part1) / 2.0);
  int div_part3 = logn - div_part1 - div_part2;
...
void Bootstrapper::ScaleDown(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{

  const auto &modulus = ckks->context->first_context_data().parms().coeff_modulus();
  auto curr_level = ckks->context->get_context_data(cipher.chain_index()).chain_depth();
  ...
  PhantomPlaintext tmpplain;
  ckks->encoder.encode(tmpvec, cipher.scale(), tmpplain);
  ckks->evaluator.mod_switch_to_inplace(tmpplain, cipher.chain_index());
  // std::cout << "tmpplain scale: "<<log2( tmpplain.scale() ) <<std::endl;
  ckks->evaluator.multiply_plain_inplace(rtncipher, tmpplain);
  // evaluator.multiply_vector_reduced_error(cipher, tmpvec, rtncipher);
  ckks->evaluator.rescale_to_next_inplace(rtncipher);
}

void Bootstrapper::slim_bootstrap_inplace(PhantomCiphertext &cipher)
{
  PhantomCiphertext rtncipher;
  slim_bootstrap(rtncipher, cipher);
  cipher = rtncipher;
}
void Bootstrapper::slim_bootstrap(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
...
void Bootstrapper::slim_bootstrap_sparse_3(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  ...
  const auto &modulus = ckks->context->first_context_data().parms().coeff_modulus();
  rtn.set_scale((double)modulus[0].value());
  ...
  rtncipher = real_part;
  rtncipher.set_scale(initial_scale * rtncipher.scale() / (double)modulus[0].value());
...
void Bootstrapper::slim_coefftoslot_full(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  ...
  ckks->evaluator.complex_conjugate(tmpct1, *(ckks->galois_keys), tmpct4);
  ckks->evaluator.add_reduced_error(tmpct1, tmpct4, rtncipher);
}

void Bootstrapper::slim_slottocoeff_full(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  PhantomCiphertext tmpct;
...
void Bootstrapper::slim_slottocoeff(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  PhantomCiphertext tmpct;
...
void Bootstrapper::slim_coefftoslot(PhantomCiphertext &rtncipher, PhantomCiphertext &cipher)
{
  PhantomCiphertext tmpct;
