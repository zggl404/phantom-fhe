...
void run_relin_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    ckks_evaluator.evaluator.relinearize(dest_multiply, relin_keys, dest_relin);
    ...
    std::cout << "Relin Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Relin Max Error: " << maxError << std::endl;
}
