...
void run_rotation_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    // = secret_key.create_galois_keys(context);
    ...
    ckks_evaluator.encryptor.encrypt(plain, ciphertext);
    ...
    ckks_evaluator.evaluator.rotate_vector(ciphertext, 5, galois_keys, rotated_ciphertext);
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    float elapsedTime;
    cudaEventElapsedTime(&elapsedTime, start, stop);
    //duration<double> sec = system_clock::now() - start;
    std::cout << "Rotation Kernel execution time: " << elapsedTime << " ms" << std::endl;
    ...
    for(int i = 0; i < input_vector.size(); ++i){
        standard_vector[i] = input_vector[(i + 5) % input_vector.size()];
    }
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(standard_vector, output_vector);

    std::cout << "Rotation Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Rotation Max Error: " << maxError << std::endl;
}
