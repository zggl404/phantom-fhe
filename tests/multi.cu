...
void run_multi_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Multi Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    std::cout << "Multi Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Multi Max Error: " << maxError << std::endl;
}
