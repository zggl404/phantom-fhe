...
void run_negation_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Negation Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    for(size_t i = 0; i < input_vector.size(); i++){
        standard_vector[i].real(-1.0 * input_vector[i].real());
        standard_vector[i].imag(-1.0 * input_vector[i].imag());
    }
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(standard_vector, output_negate);

    std::cout << "Negation Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Negation Max Error: " << maxError << std::endl;
}
