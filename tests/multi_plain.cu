...
void run_multi_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Multi plain Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(standard_vector, output_plain);

    std::cout << "Multi plain Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Multi plain Max Error: " << maxError << std::endl;
}
