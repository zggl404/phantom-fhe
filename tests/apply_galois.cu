...
void run_rotation_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    uint32_t galois_elts = context.key_galois_tool_->get_elt_from_step(3);
    ...
    std::cout << "Apply galois Kernel execution time: " << elapsedTime << " ms" << std::endl;

    ckks_evaluator.decryptor.decrypt(dest_galois, result);
    ...
    std::cout << "Apply galois Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Apply galois Max Error: " << maxError << std::endl;
}
