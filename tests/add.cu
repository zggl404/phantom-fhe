...
void run_add_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(output_add, standard_vector);

    std::cout << "Add Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Add Max Error: " << maxError << std::endl;
}
