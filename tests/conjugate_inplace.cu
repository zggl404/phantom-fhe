...
void run_conjugate_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Conjugate inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(output_conjugate_inplace, standard_vector);

    std::cout << "Conjugate inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Conjugate inplace Max Error: " << maxError << std::endl;
}
