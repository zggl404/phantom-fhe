...
void run_add_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Add many Kernel execution time: " << elapsedTime << " ms" << std::endl; 
    ...
    for(size_t i = 0; i < input1_vector.size(); i++){
        standard_vector[i] = input1_vector[i] + input2_vector[i] + input3_vector[i];
    }

    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(output_many, standard_vector);

    std::cout << "Add many Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Add many Max Error: " << maxError << std::endl;
}
