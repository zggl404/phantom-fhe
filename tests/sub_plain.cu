...
void run_sub_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Sub plain Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    std::cout << "Sub plain Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Sub plain Max Error: " << maxError << std::endl;
