...
void run_sub_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Sub plain inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(standard_vector, output_plain_inplace);

    std::cout << "Sub plain inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Sub plain inplace Max Error: " << maxError << std::endl;
