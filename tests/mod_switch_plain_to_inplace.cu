...
void run_modswitch_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    ckks_evaluator.evaluator.mod_switch_to_inplace(plain, 3);
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    float elapsedTime;
    cudaEventElapsedTime(&elapsedTime, start, stop);
    //duration<double> sec = system_clock::now() - start;
    std::cout << "Mod switch plain to inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  


    vector<complex<double>> output_inplace;
    ckks_evaluator.encoder.decode(plain, output_inplace);

    ASSERT_EQ(input_vector.size(), output_inplace.size());
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(input_vector, output_inplace);

    std::cout << "Mod switch plain to inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Mod switch plain to inplace Max Error: " << maxError << std::endl;
}
