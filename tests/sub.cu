...
void run_sub_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(standard_vector, output_sub);

    std::cout << "Sub Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Sub Max Error: " << maxError << std::endl;
}
