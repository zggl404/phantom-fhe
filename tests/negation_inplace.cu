...
void run_negation_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Negation inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  

    PhantomPlaintext result_negate;
    ckks_evaluator.decryptor.decrypt(cipher, result_negate);
    ...
    std::cout << "Negation inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Negation inplace Max Error: " << maxError << std::endl;
}
