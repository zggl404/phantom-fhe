...
void run_add_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Add plain inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    auto [sumAbsError, maxError, maxError_index] = calculateErrorStats(output_plain_inplace, standard_vector);

    std::cout << "Add plain inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Add plain inplace Max Error: " << maxError << std::endl;
}
