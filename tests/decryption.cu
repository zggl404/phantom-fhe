...
void run_encrypt_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Decryption Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    std::cout << "Decryption Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Decryption Max Error: " << maxError << std::endl;
}
