...
void run_modswitch_test(size_t poly_modulus_degree, const vector<int>& coeff_modulus, double scale){
    ...
    std::cout << "Mod switch to inplace Kernel execution time: " << elapsedTime << " ms" << std::endl;  
    ...
    std::cout << "Mod switch to inplace Mean Absolute Error (MAE): " << sumAbsError << std::endl;
    std::cout << "Max Error Index: " << maxError_index << endl << "Mod switch to inplace Max Error: " << maxError << std::endl;
}
